...
export default (appState, routed) => {
  ...
  return {
    ...
    publishEnabled: (!appState.importData.publishing && (myVres[routed.params.vreId] || {}).publishState !== PublishState.MAPPING_EXECUTION) && collectionTabs.every(tab => tab.complete),
    publishStatus: appState.importData.publishStatus ? appState.importData.publishStatus  : ( (myVres[routed.params.vreId] || {}).publishState === PublishState.MAPPING_EXECUTION ? "Publishing" : "Publish dataset"),
