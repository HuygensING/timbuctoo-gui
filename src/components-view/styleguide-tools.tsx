...
function addStyleTag(id: string, cssCode: string) {
...
function isDefaultProp(defaultProps: any, key: any, value: any) {
    if (!defaultProps) {
        return false;
    }
...
function stringifyObject(object: any, opts: { useFunctionCode?: boolean; functionNameOnly?: boolean } = {}): string {
    if (object == null) {
        ...
        Object.keys(object).map(key => {
            result += JSON.stringify(key) + ': ' + stringifyObject(object[key]) + ', ';
        });
        return result + '}';
    } else {
...
function serializeItem(item: any, options: any, delimit: boolean = true) {
    let result: string;
...
function jsxToString(component: any, options?: any) {
    const baseOpts = {
    ...
    const componentData: { name: any; props?: any; children?: any } = {
    ...
    if (component.props.children) {
        ...
        if (Array.isArray(component.props.children)) {
            componentData.children = component.props.children
                .reduce((a: any, b: any) => a.concat(b), []) // handle Array of Arrays
                .filter((child: any) => {
                ...
                .map((child: any) => serializeItem(child, opts, false))
                .join(`\n${indentation}`);
        } else {
...
export class DescribedMock extends React.Component<{ title: string }> {
    render() {
...
export class StyleGuide extends React.Component<{
    children: React.ReactElement<{ title: string }>[] | React.ReactElement<{ title: string }>;
}> {
    componentDidMount() {
        addStyles();
    }
        ...
        return (
            <div className="hds">
                <div className="hds-aside">
                    <img src="/assets/logo-knaw-humanities-cluster.png" alt="" className="hds-aside-logo" />
