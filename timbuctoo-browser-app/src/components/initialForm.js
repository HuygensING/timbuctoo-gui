...
const UrlForm = ({onSampleClick, skip, value, isFetching, errorText,closeErrorDiv }) => {
  return (

    skip ?
      <div>
        <div style={{ zIndex: 3, width: '100%', height: '100%', position: 'absolute', left: '50%', top: '50%' }}><i className="fa fa-spinner fa-spin fa-5x"></i></div>
        <form className="form" style={{ opacity: 0.5, position: 'relative' }}>{onSampleClick(value)}</form>
      </div>
      :
